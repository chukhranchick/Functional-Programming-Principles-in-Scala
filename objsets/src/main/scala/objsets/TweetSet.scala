...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val filteredSubTrees = left.filterAcc(p, acc)
  ...
  override def mostRetweeted: Tweet = {
    val mostRetweetedSet =
      left.filter(tweet => tweet.retweets > elem.retweets)
        .union(right.filter(tweet => tweet.retweets > elem.retweets))
    mostRetweetedSet match {
      case _: Empty => elem
      case m: NonEmpty => m.mostRetweeted
    }
  }

  override def descendingByRetweet: TweetList = {
    def loop(current: TweetSet): TweetList = {
      current match {
        case _: Empty => Nil
        case _: NonEmpty =>
          val head = current.mostRetweeted
          new Cons(head, loop(current.remove(head)))
      }
    }
    loop(this)
  }
