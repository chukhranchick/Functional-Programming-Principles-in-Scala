...
trait Huffman extends HuffmanInterface:
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] =
    if singleton(trees) then trees
      ...
      :: trees.tail.tail).sortWith((e1, e2) => weight(e1) < weight(e2))
  ...
  def until(done: List[CodeTree] => Boolean, merge: List[CodeTree] => List[CodeTree])
    ...
    if done(trees) then trees else until(done, merge)(trees)
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] =
    def traverse(remainTree: CodeTree, remainBits: List[Bit]): List[Char] =
      remainTree match {
        case Leaf(c, _) => if (remainBits.isEmpty) List(c) else c :: traverse(tree, remainBits)
        case Fork(l, r, _, _) =>
          if remainBits.head == 0 then traverse(l, remainBits.tail)
          else traverse(r, remainBits.tail)
      }
    traverse(tree, bits)
  /**
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] =
    def charLookup(currentTree: CodeTree)(char: Char): List[Bit] =
      currentTree match {
        case _: Leaf => List()
        case Fork(l, r, _, _) =>
