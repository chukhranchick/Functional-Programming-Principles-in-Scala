...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.groupBy(_.toLower).map((k, v) => (k, v.length)).toList.sortBy(_._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    occurrences.foldRight(List[Occurrences](List())) {
      case((char, times), acc) =>
        acc ++ (for{combination <- acc; n <- 1 to times} yield(char, n) :: combination)
    }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    (y.foldLeft(SortedMap[Char, Int]() ++ x)){
      case (acc, (yChar, yTimes)) =>
        val n = acc(yChar) - yTimes
        if n > 0 then acc.to(collection.immutable.SortedMap).updated(yChar, n)
        else acc.to(collection.immutable.SortedMap).removed(yChar)
    }.toList
